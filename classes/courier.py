...
class Courier:
    ...
    def __init__(self, request: Request, storages: Dict[str, AbstractStorage]):
        ...
        if self.__request.destination_point in storages:
            self.__to = storages[self.__request.destination_point]

    def move(self):
        """перемещает между двумя точками"""
        ...
        print(f"Курьер доставил {self.__request.amount} {self.__request.product} в {self.__request.destination_point}")

    def cancel(self):
        """возвращает товар на тот же склад, если не хватило места для размещения"""
